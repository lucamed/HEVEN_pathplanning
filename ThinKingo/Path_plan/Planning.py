...
class Planning:  # Mission으로부터 mission number를 받아 그에 맞는 제어에 넘겨줄 list를 반환해줌.
    ...
    def make_path(self, goal, left = True, right = True, lidar = True):         
            start = (350,50,np.pi/2)#self.database.imu.data[2]
            #end = (400,450,np.pi/2)#self.__local_target
            self.__map.start = start
            self.__map.end = goal

            self.__map.obstacle = self.mapping.update_map(left_on = left ,right_on = right, lidar_on = lidar)
            
            self.vehicle.set_position([start[0],start[1],start[2]])
            #vehicle.show()
            plan = HybridAStar(self.__map.start, self.__map.end, self.__map, self.vehicle, r= self.radius, r_step = 25, grid_step=10)
