...
class YOLO():
    def __init__(self, db):  # db is optional
        self.db = db
        self.flag = db.flag
        self.YOLO = threading.Thread(target=self.__main)
    ...
    def __main(self):
        ...
        while not self.flag.system_stop:  # By stoping system, Reading GPS should be stopped, too.
            if self.flag.yolo_stop:
                ...
                prev_time = time.time()
                ret, frame_read = True, self.db.parking_cam.data
                ...
                cv2.putText(image, 'Mission: ' + self.db.mission, (10, 400), font, fontScale, fontColor, lineType)
