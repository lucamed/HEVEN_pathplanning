...
def logging():
    ...
    def update(_default_img, _buffer):
        input_data = [50000 * float(db.gps.data[2]), 50000 * float(db.gps.data[3])]
        ...
        for i, item in enumerate(_buffer):
            update_img = cv2.circle(update_img, (288 + int(item[1] - current[1]), 288 - int(item[0] - current[0])), 2, (int(255 * ((len(_buffer) - i) / len(_buffer))), int(255 * (i / len(_buffer))), 0), 1)
