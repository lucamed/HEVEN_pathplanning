...
class LiDARClusterScreen(Screen):
    ...
    def render(self):
        ...
        dx = self.db.control_data.target[0] - self.db.control_data.curr_pos[0]
        dy = self.db.control_data.target[1] - self.db.control_data.curr_pos[1]
        self.__img = cv2.circle(self.__img, (int(144 + dx * 10), 162 + int(dy * 10)), 10, (0, 255, 0), 2)
        angle = self.db.control_data.curr_angle * np.pi / 180
        self.__img = cv2.line(self.__img, (144, 162), (144 + int(np.sin(angle) * 100), 162 - int(np.cos(angle) * 100)), (255, 0, 0), 1)
        self.__img = cv2.putText(self.__img, f"Angle:{self.db.control_data.curr_angle}", (288, 30), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0))
