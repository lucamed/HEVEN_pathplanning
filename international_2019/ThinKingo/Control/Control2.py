...
class Control2:
    def __init__(self, db):
        ...
        #지나친 인덱스 갯수
        self.passing_idx = 0
        self.path_plan = Path_plan(db = self.db, num = 2)
    ...
    def __main(self):
        ...
        while not self.flag.system_stop:
            ...
            if mission_num is 8:
                ...
                self.db.flag.yolo_stop = True
                obstacle_size = len(self.db.path.static_obs_2_1) - 1
                # 마지막 인덱스에 도달했다면, 미션 번호를 다시 7번으로
                if self.index_for_obs == obstacle_size:
                    mission_num = 7
                    self.db.flag.yolo_stop = False
            # =============================================================          
            ...
            if mission_num is 8:  # 주차 미션의 경우
                if packet[1] is not None:  # 주차 트랙이 주어질 경우
                    if packet[1] is 1:
                        parking_track = self.db.path.park_1.copy()
                    elif packet[1] is 2:
                        parking_track = self.db.path.park_2.copy()  
                    elif packet[1] is 3:
                        parking_track = self.db.path.park_3.copy()  
                    elif packet[1] is 4:
                        parking_track = self.db.path.park_4.copy()  
                    elif packet[1] is 5:
                        parking_track = self.db.path.park_5.copy()  
                    elif packet[1] is 6:
                        parking_track = self.db.path.park_6.copy()  # 주차 트랙 사용
            ...
            distance_detect = self.distance_detection(self.db.platform.recv_data.speed, mission_num)
            ...
            if mission_num is 8:
                ...
                if(self.passing_idx <= 10 and self.line_flag == True):
                    pass
                else:
                    self.line_flag = False
                    self.passing_idx = 0
            ...
            # *********************************************************************************************************
            '''if mission_num is 8 and packet[1] is True and self.park_flag is False:
                #parking macro start
                self.parking_macro()
                self.park_flag = True
                #final_speed, final_gear, final_steer_angle, final_brake 결정
                # -----------------------------------------------------------------
            ...
            # 직선 구간, 속도 빠르게
            if 1200 <= index <= 1389 or index >= 1876 :
                final_speed += 40
            ...
            print(f"self.line_flag{self.line_flag}, index_for_obs {self.index_for_obs}, obstacle {obstacle_track[0]}, distance {self.distance()}, angle{final_steer_angle}")
            # *********************************************************************************************************
            # 플랫폼으로 통신 완료
            # *********************************************************************************************************
            ...
            #print(f"packet : {packet}, index : {index}, angle : {final_steer_angle}")
    ...
    def __parking_macro(self):
        temp_time =time.time()
        while (time.time() - temp_time) < 1 :
            self.db.platform.send_data.gear = 0x00
            self.db.platform.send_data.speed = 0
            self.db.platform.send_data.brake = 150
            self.db.platform.send_data.steer = 1500

        temp_time =time.time()
        while (time.time() - temp_time) < 8 :
            #차량 우회전
            self.db.platform.send_data.steer = 1500
            self.db.platform.send_data.brake = 0
            self.db.platform.send_data.speed = 60
    ...
    def distance_detection(self, speed, mission_num):
        '''if speed < 100:
            return 9
        elif 100 <= speed < 150:
