...
CAR_SPEED = 120  # 차량의 기본 주행 속력 (km/h * 10)
KP = 0.5  # P gain
KD = 3  # D gain
...
class Control1:
    def __init__(self, db):
        ...
        self.passing_idx = 0
        #패스플랜 만들기
        self.path_plan = Path_plan(db = self.db, num = 1)
        # --------------------------------------------------------------------
    ...
    def __main(self):
        ...
        # 예선전에서는 2번-1번-2번 트랙
        obstacle_track = self.db.path.static_obs_1_2.copy()
        ...
        while not self.flag.system_stop:
            #mission_num = 1
            packet = self.path_plan.get_mission_packet()
            ...
            if 0 <= index <= 16:
                ...
                packet[0] = 8
                mission_num = 8
                packet[1] = self.db.path.gps_park
            # *********************************************'''
            ...
            if mission_num is 8:
                ...
                '''obstacle_size = len(self.db.path.static_obs_1_1) - 1
            ...
            car_angle = self.db.imu.data[2] - 2
            ...
            if mission_num is 8:  # 주차 미션의 경우
                if packet[1] is 1:
                    parking_track = self.db.path.parking1.copy()
                elif packet[1] is 2:
                    parking_track = self.db.path.parking2.copy()
                elif packet[1] is 3:
                    parking_track = self.db.path.parking3.copy()
                else:  # 아직 주차 칸이 정해지지 않았다면
                    pass  # pass, total_track 사용

            elif mission_num is 1:
                '''obstacle_track = self.__static_obstacle(self.db.path.static_obs_1_1, self.db.path.static_obs_1_2, obstacle_track)'''
                pass
            ...
            if mission_num is 8:
                if parking_track is not None:  # parking_track 이 있으면
                    p_targ = list(parking_track[index_for_parking])  # 타겟 점은 parking_track 에서 선택
                    packet[2] = len(parking_track)  # 주차 알고리즘을 위한 데이터 저장
                    packet[3] = index_for_parking
                else:
                    pass
            
            elif mission_num is 1:
                '''if(self.passing_idx <= 3 and self.line_flag == True):
                    pass
                else:
                    self.line_flag = False
                    self.passing_idx = 0
            ...
            if mission_num is 1:
                a = self.db.lidar.data

                for i in range(60, 150):
                    if a[i] < 2500 and a[i] != 0.0:
                        curr_angle_error += -i * 0.06

                for i in range(300, 210, -1):
                    if a[i] < 2500 and a[i] != 0.0:
                        curr_angle_error += (360-i) * 0.06
            ...
            #print(f"self.line_flag{self.line_flag}, index_for_obs {self.index_for_obs}, obstacle {obstacle_track[0][1]}, distance {self.distance()}, angle {final_steer_angle}, gps_data {self.db.gps.data[1], self.db.gps.data[3]}")
            # *********************************************************************************************************
            # 플랫폼으로 통신 완료
            # *********************************************************************************************************
            ...
            print(packet, index, final_steer_angle, curr_angle_error)
            #print(gps_data, car_angle)

            # Save current index
            self.path_plan.set_mission_idx(index)
            #print(f"Control1: {index}")
    ...
    def __do_mission(self, packet, target_speed):
        ...
        if mode_num is 0 or None:
            ...
            if 400 <= distance_from_obstacle < 600:
                brake = 30

            elif 200 <= distance_from_obstacle < 400:
                brake = 50

            elif distance_from_obstacle < 200:
                brake = 70
    ...
    def distance_detection(self, speed):
        if speed < 100:
            return 9
        elif 100 <= speed < 150:
            return 9 + 0.1*(speed - 100)
        elif speed >= 150:
            return 14
