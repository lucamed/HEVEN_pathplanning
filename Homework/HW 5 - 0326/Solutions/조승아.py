...
def draw_line(img,lines):
    ...
    x_top=(x_l1+x_r2)/2
    y_top=(y_l1+y_r2)/2
    cv2.line(img,(int(x_top),int(y_top)),(int(x_top),img.shape[0]),(0,255,0),5)
...
road=cv2.VideoCapture("WW.mp4")     #파일명 바꾸면 WW, YW 적용 가능
while(road.isOpened()):
    _,frame=road.read()
    ...
    mask=interest(canny(frame))
    lines=hough_line(mask)
    line_road=cv2.addWeighted(frame,0.8,lines,1,0)
